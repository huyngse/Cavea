  ...
  useEffect(() => {
    ...
    const getProduct = async () => {
      try {
        ...
        if (cagetypes != "") {
          res = await axios.get(
            "http://localhost:8089/regular-cage/get?birdtypeId=" + cagetypes
          );
        } else {
        ...
        console.log("Type of: ", typeof res.data.list[0].discount);
      } catch (error) {
          ...
          return item.quantity > 0 ? (
            <div className="col-lg-3 col-md-4 col-6">
              <MediaCard
                ...
                productImage={item.urlhome}
                rating={item.cageRate}
                productPrice={item.cagePrice}
                productDiscount={
                  item.cagePrice - item.cagePrice * item.discount
                }
