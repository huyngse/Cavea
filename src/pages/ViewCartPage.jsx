    ...
    const getCarts = async () => {
      try {
        const res = await axios.get(
          "http://localhost:8089/cart/get/regularCage?username=" +
            userIdC +
            "&status=0"
        );
                ...
                return (
                  <CartItem
                    ...
                    price={
                      ...
                      item.regularCages.cagePrice * item.regularCages.discount
                    }
                    cost={
                      (item.regularCages.cagePrice -
                        item.regularCages.cagePrice *
                    ...
                    cageId={item.regularCages.cageId}
                    cageShape={item.regularCages.cageShape}
                    cageMaterial={item.regularCages.cageMaterial}
                    shortDecription={item.regularCages.shortDecription}
                    birdtypeId={item.regularCages.birdtypeId}
                    cagePrice={item.regularCages.cagePrice}
                    maxQuantity={item.regularCages.quantity}
                    // totalItem={item.totalItem} Cần thêm vào
                  />
                );
              })}
              <div
          ...
          <div className="col-4 ps-5 mt-5">
            <div className="text-center mb-4">
              ...
              <h3 className="text-primary mb-3">
                {Number.parseInt(objectcart.message).toLocaleString("vi-VN", {
                  style: "currency",
                  currency: "VND",
                })}
              </h3>
