...
import CustomOrderCard from "../../components/OrderHistory/CustomOrderCard";
...
const valueMapping = { 0: "Chờ xác nhận", 1: "Thành công", 2: "Đã hủy", 3: "Đang giao", 4: "Đã giao" }

export default function UserCustomOrderPage() {
    ...
    useEffect(() => {
        ...
        const loadData = async () => {
            try {
                const response = await axios.get(`http://localhost:8089/cart/custom/history?username=${loginUser}`);
                setData(response.data.list);
                setUserInfo(response.data.object)
                setZerothData(response.data.list.filter(item => item.status === 0))
                setFirstData(response.data.list.filter(item => item.status === 1))
                setSecondData(response.data.list.filter(item => item.status === 2))
                setThirdData(response.data.list.filter(item => item.status === 3))
                setFourthData(response.data.list.filter(item => item.status === 4))
                console.log("Data from Custom History: ", response.data.list);
                console.log("User info from Custom History: ", response.data.object);
            } catch (error) {
                console.error(error);
            }
        }
        ...
        <AccountLayout>
            <Box sx={{ width: '100%' }}>
                ...
                <CustomTabPanel value={value} index={0}>
                            ...
                            _DATA0.currentData().map(item => {
                                return (
                                    <OrderCard
                                        key={item.id}
                                        // index={item.status}
                                        name={item.billId === null ? "Chưa hoàn thành" : `Mã đơn hàng: ${item.billId}`}
                                        status={item.status}
                                        total={item.price}
                                        fullData={item}
                                    >
                    ...
                    <Pagination count={count0} shape="rounded" onChange={handlePaginationChange0} page={page} />
                </CustomTabPanel>
                <CustomTabPanel value={value} index={1}>
                    ...
                    <Pagination count={count1} shape="rounded" onChange={handlePaginationChange1} page={page} />
                </CustomTabPanel>
                <CustomTabPanel value={value} index={2}>
                    ...
                    <Pagination count={count2} shape="rounded" onChange={handlePaginationChange2} page={page} />
                </CustomTabPanel>
                <CustomTabPanel value={value} index={3}>
                    ...
                    <Pagination count={count3} shape="rounded" onChange={handlePaginationChange3} page={page} />
                </CustomTabPanel>
                <CustomTabPanel value={value} index={4}>
                    ...
                    <Pagination count={count4} shape="rounded" onChange={handlePaginationChange4} page={page} />
                </CustomTabPanel>
            </Box>
        </AccountLayout>
    );
