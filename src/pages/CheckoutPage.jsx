...
export default function CheckoutPage() {
  ...
  const [isCustomizeCage, setIsCustomizeCage] = useState(false);
  ...
  const handleClick = () => {
    try {
      axios.get(
        "http://localhost:8089/cart/update-account-address?address=" +
          location +
          "&username=" +
          userIdC
      );
    ...
    if (typePay === "1") {
      if (checkCus) {
        payUrl =
          "http://localhost:8089/cart/pay?username=" + userIdC + "&status=2";
      } else {
      ...
      payUrl =
        "http://localhost:8089/cart/bill?username=" + userIdC + "&paymentID=2";
      const getCarts = async () => {
        try {
          const res = await axios.get(payUrl);
          window.open(
            "http://localhost:3000/payment-vnpay/results?vnp_ResponseCode=01"
          );
          console.log(res);
        } catch (error) {
          console.log(error.message);
  ...
  useEffect(() => {
    const getCarts = async () => {
      try {
        const res = await axios.get(
          "http://localhost:8089/cart/get?username=" + userIdC
        );
    ...
    const getCusCarts = async () => {
      try {
        ...
        if (!isCustomizeCage) {
          res = await axios.get(
            "http://localhost:8089/cart/get/custom/regularCage?username=" +
              userIdC
          );
        } else {
          res = await axios.get(
            "http://localhost:8089/cart/get/regularCage?username=" + userIdC
          );
        }
        setCusCarts(res.data.list);
        console.log("customerData: ", res.data.list);
        res.data.list.map((item) => (sum += item.price));
      } catch (error) {
        console.log(error.message);
      }
    };
                  ...
                  {!isCustomizeCage
                    ...
                    : cuscarts.map((item, index) => (
                        <tr key={index}>
                          ...
                          <td>
                            <div>
                              <span className="h6">Chất liệu: </span>
                              {item.material}
                            </div>
                            <div>
                              <span className="h6">Hình dáng: </span>
                              {item.shape}
                            </div>
                            {/* <div>
                      <span className="h6">Số lượng: </span>
 ...
 */}
              ...
              <div className="text-end">
                ...
                {isCustomizeCage ? (
                  ...
                  <span className="text-primary h3">
                    {Number.parseInt(objectcart.message).toLocaleString(
                      "vi-VN",
                      {
                        style: "currency",
                        currency: "VND",
